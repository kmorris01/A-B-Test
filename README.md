...
***Invariant Metrics--
  ...
  Number of user-ids--this one is neither invariant nor evaluation metric. Due to the enrollment could be dependent upon how we carry     out the "start free trial" page, we could expect to see different values in the control and experiment group. Also, the number of       visitors between experiment are likely to be different & number of enrolled users can fluctuate in a particular day, which likely       could skew the results. To explain further of why could skew our results is because a raw count can't adjust to the different groups     sizes (it can't normalize). This is why we use gross conversion, because it is a proportion and can normalize. 
...
# Summary--
I completely understand the importance to correct if a test is launched and the metrics show a significant difference, because it's more likely that one of multiple metrics will be falsely positive as the number of metrics increases. However, we would only launch if all evaluation metrics show a significant change. In that case, there would be no need to use Bonferroni correction for this experiment, in addition it can actualy do harm by increasing false negatives/positives. Meaning, whenever multiple comparisons are made (multiple tests) the chance of a statistical error (false negatives and false positives) increases proportionality to the number of tests, therefore I selected not to use the Bonferroni correction.  We could of also set up our experiment to consider other strategies, such as using a higher confidence level than the 95% used for each metric.  Possibly, a 99% confidence level. 



# Recommendation--
Based on the data analysis above, I recommend not to move forward with the changes of adding "5 or more hours" recommendation to "start free trial" date. The reason is that the A/B test shows that this will not practically significantly increase the net conversion rate. Meaning, it does not increase the number of paid users, which fails the original goal of our experiment/test.  In addition, the confidence interval does include a negative number of the practical significance boundary; meaning, it's possible that this number went down by an amount that could hurt the business - decrease the revenue. If we consider the initial hypothesis, it does not increase numbers of paid users, which fails the inital goal of launching this feature and likely to be a unacceptable risk to launch.  Furthermore, our gross conversion result is a good outcome because the cost is lower (meaning Udacity Coaches would be free to help those users who are dedicated) by discouraging users who are unwilling to be engaged and relates back to our Launch Criteria.
